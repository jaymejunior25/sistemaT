...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_samples'])) {
    $idsToDelete = $_POST['selected_samples'] ?? [];

    if (!empty($idsToDelete)) {
        $placeholders = implode(',', array_fill(0, count($idsToDelete), '?'));
        $stmtDelete = $dbconn->prepare("DELETE FROM pacotes WHERE id IN ($placeholders)");
        if ($stmtDelete->execute($idsToDelete)) {
            $successMessage = "Amostras excluídas com sucesso!";
        } else {
            $errorMessage = "Erro ao excluir amostras.";
        }
    }
}
...
<body>
    <div class="container container-custom2">
            ...
            <table class="table table-bordered table-striped">
                <thead class="theadfixed">
                    <tr>
                        ...
                        <?php foreach ($available_columns as $column_key => $column_label): ?>
                            <?php if (in_array($column_key, $colunas_selecionadas)): ?>
                                <th><?php echo $column_label; ?></th>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($pacotes as $pacote): ?>
                        <tr>
                        <td><input type="checkbox" name="selected_samples[]" value="<?php echo $pacote['id']; ?>"></td>
                            <?php foreach ($available_columns as $column_key => $column_label): ?>
                                <td>
                                    
                                    <?php
                                    if (in_array($column_key, $colunas_selecionadas)){
                                        if ($column_key == 'data_cadastro' || $column_key == 'data_envio' || $column_key == 'data_recebimento' || $column_key == 'data_recebimentolab') {
                                            $data = $pacote[$column_key];
                                            if ($data) {
                                            ...
                                            echo htmlspecialchars($pacote[$column_key]);
                                        }
                                    }
                                    ?>
                                </td>
        ...
        <button type="submit" name="delete_samples" class="btn btn-danger">Excluir Selecionadas</button>
        </form>
    </div>
    ...
    <script>
         document.getElementById('select_all').onclick = function() {
            var checkboxes = document.getElementsByName('selected_samples[]');
