...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['confirm_password'])&& isset($_POST['codigobarras'])) {
    ...
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        // Verificar se o pacote pertence ao mesmo local do usuário logado
    $query = $dbconn->prepare('SELECT * FROM pacotes WHERE codigobarras = :codigobarras AND unidade_cadastro_id = :unidade_cadastro_id');
    $query->execute(['codigobarras' => $codigobarras, 'unidade_cadastro_id' => $local_envio_id]);
    $pacote = $query->fetch(PDO::FETCH_ASSOC);

    if (password_verify($_POST['confirm_password'], $user['senha'])) {
        if ($pacote) {
            // Atualizar o status do pacote para "Enviado"
            $stmt = $dbconn->prepare("UPDATE pacotes SET status = 'enviado', data_envio = NOW(), unidade_envio_id = :unidade_envio_id, 
            usuario_envio_id = :usuario_envio_id WHERE codigobarras = :codigobarras");
            $stmt->execute([':unidade_envio_id'=> $_SESSION['unidade_id'], ':usuario_envio_id' => $_SESSION['user_id'], ':codigobarras' => $codigobarras]);

            $_SESSION['success_message'] = 'Pacote enviado com sucesso.';
           // header('Location: index.php');
            //exit();
        } else {
            $_SESSION['error_message'] = 'Pacote não encontrado ou você não tem permissão para enviar este pacote.';
        }
    } else {
...
<body>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="confirmForm" method="post" action="">
                            <input type="hidden" name="codigobarras" id="hidden_codigobarras">
                            <div class="form-group">
    ...
    <script>
        function confirmAction(event) {
            ...
            document.getElementById('hidden_codigobarras').value = codigobarras;
